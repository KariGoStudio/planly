...
export default function SwipeCard({ item, onLike, onDislike, onSave, isTop, moodCopy }: Props) {
  ...
  async function animateOut(dir: "left" | "right") {
    ...
    setSwiping(true);
    await animate(x, dir === "right" ? 700 : -700, { duration: 0.3, ease: "easeOut" });
    if (dir === "right") onLike();
    else onDislike();
  }
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-7 pointer-events-none">
          <div className="flex items-start justify-between">
            <div className={["select-none leading-none", showImage ? "text-5xl mt-2 drop-shadow-lg" : "text-7xl mt-2"].join(" ")}>
              {item.emoji}
            </div>
            {moodCopy && (
          ...
          <div>
            <h2 className="text-2xl font-bold text-white leading-tight drop-shadow">{item.title}</h2>
            <p className="text-white/70 text-sm mt-1">{item.subtitle}</p>
            {item.meta && <p className="text-white/50 text-xs mt-0.5 font-medium">{item.meta}</p>}
            <p className="text-white/85 text-sm mt-3 leading-relaxed line-clamp-2 drop-shadow">{item.description}</p>
            <div className="flex flex-wrap gap-1.5 mt-4">
              {item.tags.map((tag) => (
                <span key={tag} className="px-3 py-1 rounded-full bg-white/15 text-white/85 text-xs font-medium backdrop-blur-sm">
                  {tag}
                </span>
              ))}
            </div>
